...
# source_data = 'data/source_data/responsive-addresses.data' emb_data_file
...
class RFCBased(object):
    def __init__(self, batch_size,rfc_profile, source_data, rfc_data_path, rfc_id_path,source_file):
        self.classify_dict = {}
        self.cmd = 'cat {} | ../../Tools/ipv6toolkit/addr6 -i -d > {}'.format(source_file, rfc_profile)
...
class EntropyClustering(object):
    def __init__(self, batch_size,ec_profile, ec_cluster, source_data, ec_data_path, ec_id_path, k=4):
        self.classify_dict = {}
        self.classify_prefix_dict = {}
        self.k = k
        self.profile_cmd = 'cat {} | '.format(source_data) + \
                           '../../Tools/entropy-clustering/profiles > {}'.format(ec_profile)
        self.cluster_cmd = 'cat {} | '.format(ec_profile) + \
                           '../../Tools/entropy-clustering/clusters -kmeans -k ' \
                           + str(k) + ' > {}'.format(ec_cluster)
...
class IPv62Vec(object):
    def __init__(self, batch_size,source_data, ipv62vec_profile, ipv62vec_data_path, ipv62vec_id_path):
        self.source_data = source_data
