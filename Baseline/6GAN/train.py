...
candidate_path = out+"candidate_set/"
...
def seed_classification(method_id=0, classifier=RFCBased(BATCH_SIZE,rfc_profile, emb_data_file, rfc_data_path, rfc_id_path,work_file)):
    if method_id == 0:
        print("Classification method: RFC Based")
        classifier = RFCBased(BATCH_SIZE,rfc_profile, emb_data_file, rfc_data_path, rfc_id_path,work_file)
    elif method_id == 1:
        print("Classification method: Entropy Clustering")
        classifier = EntropyClustering(BATCH_SIZE,ec_profile, ec_cluster, emb_data_file, ec_data_path, ec_id_path, k=6)
    elif method_id == 2:
        print("Classification method: IPv62Vec")
        classifier = IPv62Vec(BATCH_SIZE,emb_data_file, ipv62vec_profile, ipv62vec_data_path, ipv62vec_id_path)
    elif method_id == -1:
...
def main():
    ...
    # data pre-processing
    # source——data只需要一个txt后缀的文件，里面是一行一条数据，就可以生成其他的文件
