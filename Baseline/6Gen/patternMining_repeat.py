...
import numpy as np
import argparse, code, time, math, resource,os,re,random
from ActiveScanG import scan_addr
bit_list=['0','f']
...
def get_density_item(IPs, pattern):
    ...
    if len(var_index_list) == 0:
        return []
    # print('var_index_list {}'.format(var_index_list))
    var_patterns = [] # 变化部分
    for IP in IPs:
        s = ''.join([IP[i] for i in var_index_list])
        var_patterns.append(s)
    # print('var_patterns {}'.format(var_patterns))
    ...
    # print('min {} max {}'.format(min_pattern, max_pattern))
...
def findMaxLength(pattern, limit = 4):
    ...
    # print('findMaxLength temp {}'.format(temp))
    if len(temp) == 0:
        Length = limit
    else:
...
def process(IPs, budgets, depth, filename, prior_experience, limit, retrain, increase):
    ...
    while True:
        ...
        while 1:
            ...
            # print(buckets)
            # print(len(buckets[currentBucket][0][0]))
            # quit(0)
            if bucketCount == 0: 
                # print('a')
                currentBucket += 1
    ...
    for i in range(budget_index, len(budgets)):
        budget = budgets[i]
        predict(filename, budget, TargetList)
...
def predict(filename, budget, TargetList):
    ...
    f2=open(filename+'pattern.txt', 'w')
    writecount = 0
    exactcount = 0.0
    for item in TargetList:
        density, pattern = item[0], item[1]
...
def scan(sub_output,file,len_detail=[0,0,0]):
    # scan and analysis
...
def delete_repeat_ip(generate_samples,origin_samples,sub_output,budget):
    # generate_samples = open('Baseline/6Gen/dataset1/2320000/generate_samples.txt','r').readlines()
    # origin_samples = open('Baseline/Database/dataset1.txt','r').readlines()
    generate_samples = open(generate_samples,'r').readlines()
    origin_samples = open(origin_samples,'r').readlines()
    generate_samples = [i.strip() for i in generate_samples]
    origin_samples = [i.strip() for i in origin_samples]
    generate_samples_set = set(generate_samples)
    origin_samples_set = set(origin_samples)
    
    # 1. 去重
    generate_samples_unique = generate_samples_set - origin_samples_set
    
    unique_output = sub_output + '/generate_samples_unique.txt'
    os.makedirs(os.path.dirname(unique_output), exist_ok=True)
    with open(unique_output,'w') as f:
        f.write('\n'.join(generate_samples_unique))
    f.close()

    # 2. 随机抽取
    if len(generate_samples_unique) > budget:
        generate_samples_unique_random = random.sample(generate_samples_unique, budget)
    else:
        generate_samples_unique_random = generate_samples_unique
    ...
    with open(output,'w') as f:
        f.write('\n'.join(generate_samples_unique_random))
    f.close()

    return [len(generate_samples_set),len(generate_samples_unique),len(generate_samples_unique_random)]
...
#     # parse.add_argument('--read',type=str,defaulyt ='' ,help='input hex IPv6 . # to comment \\n to split')
...
#     parse.add_argument('--limit',type=int,help='split limit ',default= 4)
...
#     args.read = 'Baseline/Database/dataset1_no_colons.txt'
#     args.read_origin = 'Baseline/Database/dataset1.txt'
    
#     args.budgets = [prefix_num*i for i in num]
#     args.write = 'Baseline/6Gen/dataset1_repeat/'
...
#         # if budget == 100:
#         #     adjust_budget = budget*8
#         # elif budget == 1000:
#         #     adjust_budget = budget*4
#         # else:
#         #     adjust_budget = budget*2
#         adjust_budget = budget*10
#         # print('budget {} adjust_budget {}'.format(budget, adjust_budget))
#         process(IPs, [adjust_budget], args.depth, write, (args.experience == 'True'), limit, retrain, increase)
#         trans(write+'pattern.txt',write+'generate_samples_repeat.txt')
#         scan(write,write+'generate_samples_repeat.txt')
#         len_detail = delete_repeat_ip(write+'generate_samples_repeat.txt',args.read_origin,write,budget)
        
#         scan(write,write+'generate_samples.txt',len_detail)
