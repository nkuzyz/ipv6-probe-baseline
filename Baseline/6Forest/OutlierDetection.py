...
# Before a round of space partition, it builds a
# forest structure for each region and exploits an enhanced isolation
# forest algorithm to remove the outlier addresses.
# Finally, it pre-
# scans samples from the divided address regions and based on the
# results generates IPv6 addresses.
def OutlierDetect(arrs)-> Tuple[int, list]:
    if len(arrs) == 1:
        # return [], [arrs]
        return 0,[],[]
    ...
    for p in patterns:
        m,s=show_regions(p)
        pattern_module.append(m)
        pattern_sample.append(s)
    ...
    return count,pattern_module,pattern_sample

# for test
def showRegionAndOutliers(region, outliers):
    # fw = open("6Forest/result.txt",'w')
