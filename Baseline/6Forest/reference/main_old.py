from useless.Fores_ret import Start
...
for b in budget:
    ...
    # p=prefixs[0]
        ...
        # output = "6Forest/output_"+str(budget)+"/few_"+str(p)
        # result.append([p,Start(input,output,budget)])
    times = time.asctime().replace(' ','_')

    input = "Baseline/Database/su_seed_463"
    output = "Baseline/6Forest/output_su/output_"+str(b)+"/total_"+str(times)
    result.append([0,Start(input,output,b)])

    filename = "Baseline/6Forest/output_su/output_"+str(b)+"/total_"+str(times)+"/analydata.txt"
