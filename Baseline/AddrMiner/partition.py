...
def get3bgp(seeds_path):
    bgps = {}
    ansdb = pyasn.pyasn("Baseline/AddrMiner/data/ipasn.dat")
    ...
    with open("Baseline/AddrMiner/data/ipasn.dat") as f:
        lines = f.readlines()
    ...
    with open("Baseline/AddrMiner/BGP/BGP-N.pk","wb") as f:
        pickle.dump(bgp_n,f)
    
    with open("Baseline/AddrMiner/BGP/BGP-N","w") as f:
        for bgp in bgp_n:
            f.write(bgp+"\n")
     
    with open("Baseline/AddrMiner/BGP/BGP-F.pk","wb") as f:
        pickle.dump(bgp_f,f)
    
    with open("Baseline/AddrMiner/BGP/BGP-F","w") as f:
        for bgp in bgp_f:
            f.write(bgp+" "+str(len(bgp_f[bgp]))+"\n")
        
    with open("Baseline/AddrMiner/BGP/BGP-S.pk","wb") as f:
        pickle.dump(bgp_s,f)
    
    with open("Baseline/AddrMiner/BGP/BGP-S","w") as f:
        for bgp in bgp_s:
            f.write(bgp+" "+str(len(bgp_s[bgp]))+"\n")

if __name__ == "__main__":
    get3bgp("Baseline/AddrMiner/data/seeds.csv")