...
import pandas as pd
# import secrets
import subprocess,json, time,re,random
import sys,os
import SubnetTree
def makedirs(path):
...
def Scan(addr_set, source_ip, output_file, tid=0):
    ...
    makedirs(os.path.dirname(scan_output))
    makedirs(os.path.dirname(scan_input))

    with open(scan_input, 'w') as f:
    ...
    if len(addr_set) != 0:
        hit_rate = float(len(active_addrs)) / len(addr_set)
    else:
        hit_rate = 0
    print('[+]hit rate = {}'.format(hit_rate))
    return len(addr_set),len(active_addrs),hit_rate

def scan_addr(output_file,datafile):
    ...
    # check line is not empty
        while  line:
            ...
            line  =  fileHandler.readline()
    return Scan(addr_set,source_ip,output_file)
...
# print(get_entire_ipv6('2a00:2380:300e:400f::f'))
...
def generate_ipv6_from_prefix(prefix_list):
    ...
    for prefix in prefix_list:
        ...
        for i in range(16):
            rand_bytes = os.urandom((left_length + 1) // 2)
            rand_hex = rand_bytes.encode('hex')[:left_length]
            ipv6s[i] = ipv6s[i] + hex(i)[2:] + rand_hex
            ipv6s[i] = add_ipv6_colon(ipv6s[i])
        generate_samples+=ipv6s
    return generate_samples


# print(generate_ipv6_from_prefix(['20014234']))
...
def delete_alias_ip(alias,ip_samples,path):
    ...
    for line in ip_address_file:
        ...
        try:
            ...
            # f.write(line+"\n")
            # print('[-]No alias found for {}'.format(line))
...
def scan(sub_output,path):
    # scan and analysis
    
    result = []
    result.append([0,scan_addr(sub_output,path)])
...
#     output = '/home/chengdaguo/ipv6/Baseline/Entropy-ip/result_few'
#     file = output+'/ipv6.txt'
...
#     file_updata = output+'/ipv6_updata.txt'
#     f1 = open(file_updata,'w')
#     f1.write('\n'.join(origin_samples))
#     f1.close()


#     scan(output,file_updata)


#     test_data = open(output+'/zmap/scan_output_0.txt').readlines()
#     sub_output = output+'/alias'
