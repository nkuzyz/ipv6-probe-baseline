...
def TopK(ipv6_list, PD, k=5, budget=None):  #kheap
    ...
    return random.sample(new_ipv6,int(budget)) if budget < len(new_ipv6) else new_ipv6
...
def get_good_p(multi_level, hmin=14.0, hmax=16.0, algorithm="louvain", sst=1e7, types=4, emin=0.4, emax=0.8):
    ...
    with open("/home/chengdaguo/ipv6/Baseline/AddrMiner-main/pk_data2/matrix.pk","wb") as f:
...
def getPD(num):
    pdpath="Baseline/AddrMiner-main/pk_data{}/good_dis_p.pk".format(num)
...
if __name__ == "__main__":
    ...
    # os.makedirs("/home/chengdaguo/ipv6/Baseline/AddrMiner-main/pk_data2",exist_ok=True)
    print('[+]data loading..')
    multi_level = dataloader("./data/dataset2.csv", "./data/ipasn.20230522.dat")
    print('[+]undirected graph creation, get pattern library..')
    good_dis_p = get_good_p(multi_level,
            hmin=args.hmin,
            hmax=args.hmax,
            ...
            emin=args.emin,
            emax=args.emax)
    print('[+]get pattern library done!')
    with open("/home/chengdaguo/ipv6/Baseline/AddrMiner-main/pk_data2/good_dis_p.pk","wb") as f:
