...
if __name__ == '__main__':
    ...
    for num in nums:
        ...
        for budget in budgets:
            ...
            sub_output = dataset+'no_{}'.format(budget)
            print(sub_output)
            ...
            # 遍历一个文件夹中的以“scan_output_”开头的文件
            alias_len = 0
            for root, dirs, files in os.walk(sub_output+'/zmap'):
                for file in files:
                    if file.startswith('scan_output_'):
                        print(os.path.join(root, file))
                        origin_samples = open(os.path.join(root, file)).readlines()
